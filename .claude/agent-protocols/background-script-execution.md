...
Follow `.claude/scripts/sbt-run.sh` as the reference implementation:
...
2. Resolve `SCRIPT_DIR` / `REPO_ROOT` the same way every other script in
   `.claude/scripts/` does (`cd "$(dirname "${BASH_SOURCE[0]}")" && pwd`, then
   `../..`).
...
parameterized abstraction — see `.claude/scripts/sbt-run.sh` (generalizes
