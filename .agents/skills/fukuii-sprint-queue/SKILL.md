...
Dispatches to the three collector scripts in `scripts/agent-tooling/` that keep
...
### status
```bash
scripts/agent-tooling/sprint-status.sh
```
...
### clear
```bash
scripts/agent-tooling/sprint-clear.sh          # dry run — always show this first
```
If it reports CLOSED batches ready to move, show the preview to the operator and
confirm before applying:
```bash
scripts/agent-tooling/sprint-clear.sh --apply
```
...
### archive
```bash
scripts/agent-tooling/sprint-archive.sh <file>          # dry run — always show this first
```
...
```bash
scripts/agent-tooling/sprint-archive.sh <file> --apply
```
