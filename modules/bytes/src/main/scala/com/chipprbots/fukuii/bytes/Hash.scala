...
/** A 32-byte value — a keccak-256 digest, a block/tx hash, an MPT node hash, or any fixed 32-byte word.
  *
  * Type-distinct opaque wrapper over its backing 32 bytes; a `Hash` cannot be silently mixed with an [[Address]]. The
  * primary constructor [[apply]] is **strict**: it requires exactly 32 bytes and fails loud otherwise. The lenient geth
  * `SetBytes` reshaping is the explicitly-named [[fromBytesTruncating]]. [[Bytes32]] is a transparent alias for the
  * same type where the 32-byte width matters more than the "hash" role (e.g. an EVM/storage word as bytes).
  */
...
object Hash:
  ...
  /** Wrap **exactly** 32 bytes; throws on any other length. Use [[fromBytesTruncating]] for the parse/derive sites that
  ...
  /** Right-align raw bytes into exactly 32 bytes, matching go-ethereum `common.Hash.SetBytes` (`common/types.go`)
    * byte-for-byte: a longer input keeps its rightmost 32 bytes, a shorter one is left-padded with zeros — matching
    * `BytesToHash`. For boundary/parse sites only.
    */
  ...
  /** Parse from hex, tolerating a `0x` prefix; must decode to at most 32 bytes. A shorter value is left-padded
    * (matching `BytesToHash`); an oversized value is rejected fail-loud.
    */
