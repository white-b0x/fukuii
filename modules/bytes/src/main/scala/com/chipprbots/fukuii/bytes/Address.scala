...
/** A 20-byte Ethereum account address.
  *
  * Type-distinct opaque wrapper over its backing 20 bytes — an `Address` cannot be silently mixed
  * with a [[Hash]] even though both wrap `ByteString`. The primary constructor [[apply]] is
  * **strict**: it requires exactly 20 bytes and fails loud otherwise. The lenient geth `SetBytes`
  * reshaping (right-align/left-pad/truncate) is the explicitly-named [[fromBytesTruncating]], for
  * the boundary sites that need it.
  */
...
object Address:
  ...
  /** Wrap **exactly** 20 bytes; throws on any other length. Use [[fromBytesTruncating]] for the
  ...
  /** Right-align raw bytes into exactly 20 bytes, matching go-ethereum `common.Address.SetBytes`
    * (`common/types.go`) byte-for-byte: a longer input keeps its rightmost 20 bytes, a shorter one is
    * left-padded with zeros. For boundary sites only — RLP-decoding an address field, or deriving an
    * address from the low 20 bytes of a 32-byte word (CREATE/CREATE2, `ecrecover`, `BytesToAddress`).
    */
  ...
  /** Parse from hex, tolerating a `0x` prefix; must decode to at most 20 bytes. A shorter value is
    * left-padded, matching go-ethereum `HexToAddress`; an oversized value is rejected fail-loud.
    */
