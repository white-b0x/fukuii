...
/** The layer a proposal lives in — drives the validation gate and ownership (Batch 5 framework §1.6).
  ...
  *   - `Consensus`   — state-affecting. A divergent client produces a different state root and forks the chain.
  *                     forge (PoW) / beacon (PoS) own it; byte-identity + ETC state-root compliance is the gate.
