...
| **BUILD-1** | (build) | — | `sbt <module>/test` silently reports **0 tests** (false-green) under sbt 2.0.2 — confirmed with evidence | — | warden: root-cause + fix; sanctioned path meanwhile = `Test/`-scoped aliases + `testOnly` |
...
| Module | Verdict |
|---|---|
| `bytes` | GREEN (value types/hex/ordering byte-exact) — pending F-UINT-1 (arithmetic completion) |
| `common` | GREEN |
| `rlp` | **NOT-YET-GREEN** — F-RLP-1 (HIGH), F-RLP-2/3 (MED), GAP-3 (LOW): encode byte-exact, decode non-strict |
| `crypto` (keccak/secp256k1/hashes/KZG/BLS) | GREEN — byte-exact; GAP-2 is a test-coverage gap |
| `crypto/zksnark` (alt-bn128) | **NOT-YET-GREEN** — F-BN-1 (CRITICAL) |
...
**Not yet consensus-sound** — one CRITICAL (F-BN-1) + one HIGH (F-RLP-1) are adversarially-reachable
divergences from the byte-authority, both flagged by the observations doc as things to check
("subgroup checks", "strict-decode / minimal-length scalars"). The value-type layer, keccak,
secp256k1 recovery/address-derivation, and the native KZG/BLS wrappers **are** byte-exact and
trustworthy on honest input.

**Resolution (before L1):** `forge` closes F-BN-1 (consensus-critical, byte-perfect vs core-geth),
F-RLP-1/2/3, GAP-2, F-UINT-1, GAP-3 in an L0-remediation pass; `warden` fixes BUILD-1; `eye`
re-validates. L0 is not "done" until every module is GREEN. The gate caught a chain-splitting bug
before we built on top of it — the reason it exists.
