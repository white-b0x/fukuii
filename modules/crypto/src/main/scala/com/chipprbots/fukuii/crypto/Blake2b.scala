...
/** BLAKE2b `F` compression function — the primitive behind the EIP-152 `BLAKE2F` precompile (address `0x09`).
  *
  * This is the round-reduced compression function `F` from the BLAKE2 RFC (RFC 7693 §3.2), not a full BLAKE2b hash: the
  * caller supplies the round count, state vector, message block, offset counters and final-block flag, and receives the
  * updated 64-byte state. The 213-byte serialized input layout is `[4 rounds][64 h][128 m][8 t_0][8 t_1][1 f]` with the
  * round count as a 32-bit big-endian word and the state/message/counter words as little-endian 64-bit words.
  *
  * Byte-behaviour is identical to go-ethereum `core/vm/contracts.go` `blake2F` (backed by `crypto/blake2b`) and besu
  * `Blake2bfMessageDigest`: same `IV`, same `PRECOMPUTED` message-permutation schedule, same `mix` rotations (`rotr`
  * 32/24/16/63), and the same `v(14) ^= 0xff…ffL` final-block flag. This object is primitive-only — gas, the `0x09`
  * address and precompile framing live in the L3 wrapper.
  */
object Blake2b:
  ...
  /** Validate the 213-byte serialized input: exact length, and the final-block flag byte (offset 212) must be `0x00` or
    * `0x01` — any other value is rejected (EIP-152).
    */
  def isValidInput(input: Array[Byte]): Boolean =
    !(input.length != MessageBytesLength || (input(212) & 0xfe) != 0)

  /** The unsigned 32-bit big-endian round count from the first 4 bytes of the input. */
  ...
  /** Run the BLAKE2b `F` compression over a valid 213-byte input, returning the updated 64-byte state as little-endian
    * bytes, or `None` if the input is malformed (see [[isValidInput]]).
    */
  def compress(input: Array[Byte]): Option[Array[Byte]] =
    if isValidInput(input) then
      val (rounds, h, m, t, f) = parseInput(input)
      compressBlock(rounds, h, m, t, f)
  ...
  private def compressBlock(rounds: Long, h: Array[Long], m: Array[Long], t: Array[Long], f: Boolean): Unit =
