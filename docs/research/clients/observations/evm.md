...
|---|---|---|---|---|---|---|---|---|
| **Fork-conditional dispatch** | immutable **jump-table-per-fork** picked once at `NewEVM` by a priority-ordered `switch` over flat `params.Rules` booleans; loop never branches on fork | **additive per-EIP** — `instructionSetForConfig` builds one table gating each opcode on `IsEnabled(GetEIPxTransition, bn)`; **no named-fork table, no `chainRules` struct** | **GasCalculator-as-strategy** object per fork + per-fork `OperationRegistry`, assembled by a factory; fork = a trio of objects, no flag struct | geth's jump-table-per-fork model, table resolved once from `chainRules` at `NewEVM` | **`IReleaseSpec`-flags** — ~100 `IsEipNNNEnabled` booleans + intent-named getters, resolved once per block from `ISpecProvider` | **`SpecId` enum + `ConfigureEvm`** — every family's `Spec: Into<SpecId>`; family bound at compile time by associated types | **Two coexisting mechanisms.** (1) additive **proposal fold** — `EvmProposals` registers per-EIP/ECIP `EvmProposal{opcodeDelta,feeDelta,configDelta}` keyed by family-agnostic `ProposalId`; `EvmConfig.deriveEvmConfigAt` folds active set → opcode list + fee schedule + flags (`EvmConfig.scala:57-79`, `EvmProposals.scala:148-155`). (2) older runtime **enum-fork lookup** still gates EIP-2929 gas + precompiles (`BlockchainConfigForEvm.scala:81-97`). Two `forBlock` overloads: 2-arg block (PoW) / 3-arg block+timestamp (PoS) (`:35-49`) | **go-ethereum** (canonical) semantics; **besu** = JVM structural model fukuii should mirror; **core-geth** = ETC EIP-groupings |
| **Gas-schedule modelling** | per-fork jump table carries `constantGas` + `dynamicGas` closures; two-dimensional `GasCosts{Regular,State}` for EIP-4762 (in-flight) | inherits geth's gas closures, gated per-EIP transition; **no EIP-1559 base-fee** in upstream ETC config | **entire schedule = one `GasCalculator` subclass**; fork diff = the overridden methods; injected into every opcode + precompile | **multidimensional `mdgas.MdGas{Regular,State}`** threaded through `Run` + every gas func (EIP-8037), with signed refunds + spill semantics | pluggable **`IGasPolicy<TSelf>`** (static-abstract) so EIP-8037 state-gas is a separate policy struct, JIT-specialized | revm owns the gas schedule; reth only configures spec/env | **besu-style fee-value inheritance chain, data-only.** `FeeSchedule` subclasses override only changed BigInt fields (`FrontierFeeSchedule`→…→`EthOsakaFeeSchedule`, `EtcOlympiaFeeSchedule`, `EvmConfig.scala:265-387`); gas *computation* stays in opcodes (`baseGas+varGas`, `OpCode.scala:260-261`). Single-dimensional `BigInt` gas. Parallel `FeeScheduleValues` case class = the fold's `.copy` carrier (`EvmProposals.scala:53-140`) | **besu** (subclassable strategy object — the fukuii model); **go-ethereum** for values/semantics |
| **Opcode-table assembly** | **copy-then-mutate** — each fork constructor copies prior fork's `[256]*operation` + applies named `enableNNNN` mutators; validated at init | **copy-then-mutate additively** from a flat base, gating each mutation on its own EIP block | **incremental registry** — each `registerXOperations` calls prior fork's then `put`s the delta into a dense `Operation[256]` | geth copy-then-mutate; ExtraEips copy-on-extend only | **function-pointer table** (`delegate*<…>[256]`) built per-fork by `GenerateOpCodes(spec)`, **memoized on the spec object** | revm-internal (handler-composition per fork) | **copy-then-prepend named lists, derived by the fold.** `OpCodes.*` bundles built additively, network-prefixed `Eth*`/`Etc*` (`OpCode.scala:150-195`); `EvmProposals.deriveEvm` reproduces them (byte-identity proven by `EvmProposalDerivationSpec`). Final table = `Map[Byte, OpCode]` (`EvmConfig.scala:150-151`), not a dense `[256]` array; no build-time `validate` | **besu**/**go-ethereum** (per-fork table built once); **nethermind** for the cache-table-on-config idea |
| **Precompile handling** | per-fork `map[Address]PrecompiledContract` selected by same fork flags; config baked statelessly into struct per fork | inherits geth's per-fork precompile maps | `PrecompileContractRegistry` (`Map<Address,…>`), incremental per fork, takes the fork's `GasCalculator` (**not applied uniformly** — some hardcode cost) | geth's `PrecompiledContract` iface, unchanged | `IPrecompile` (static-virtual `Address`) with `BaseGasCost/DataGasCost(IReleaseSpec)` fork-gated | revm/alloy `PrecompilesMap` (mutable set), configured via `ConfigureEvm` | **per-fork `Map[Address, PrecompiledContract]`, incremental** (`byzantiumAtlantisContracts = contracts ++ Map(...)`, `PrecompiledContracts.scala:49-104`); selected by `etcFork`/`ethFork` enum + timestamp + `isEthereum` guard (`:141-168`). `etcOlympiaContracts` excludes 0x0a KZG (no EIP-4844 on ETC). Precompile gas fork-gated via `gas(input, etcFork, ethFork)`. Adds `precompileRelocations` remap layer (`:121-135`) | **go-ethereum** (address→contract map, fork-selected) is the canonical shape |
| **Tracing hooks** | passive `tracing.Hooks` struct, zero-cost when nil (hoisted `debug` flag, `HasGasHook` short-circuit) | inherits geth `tracing.Hooks` | `OperationTracer` all-`default{}` iface + `NO_TRACING` singleton; JIT elides when monomorphic | geth-compatible `tracing.Hooks` (`cost` deliberately regular-gas-only to avoid tracer underflow) | **generic-tracer specialization** — `IFlag`/`OnFlag`/`OffFlag` + `static virtual` fold `if(TTracingInst.IsActive)` away → **zero hot-loop cost**; separate traced/non-traced tables | revm **`Inspector`** — tracing = "an `Evm` built with an Inspector"; same path serves `debug_*`/archival; `NoOpInspector` default | **all-default-no-op `ExecutionTracer` trait** (explicitly besu-`OperationTracer`-modelled, `ExecutionTracer.scala:9-40`). Impls: StructLog/Call/Prestate/Vm. **Not branch-free** — `tracer.foreach(_.onStep)` from two slots (VM ctor + `env.tracer`) every opcode (`VM.scala:237-238`) | **nethermind** (zero-cost generic tracer, CLR-specific) / **reth** (`Inspector` seam) — both inspirations for a branch-free disabled path |
| **Standalone-lib reusability** | `core/vm` is an internal package (not separately published) | internal package (geth-derived) | **✓ independently-published `org.hyperledger.besu:evm`** — depends only on datatypes/tuweni/Guava; embeddable by external tooling | relocated to `execution/vm/`, still an internal package | `Nethermind.Evm` internal assembly | **✓ revm** — the dominant standalone Rust EVM (Foundry, reth, many others); reth's own EVM crate <2k LoC | **✗ internal root-module package**; no published EVM artifact, no EVMC/revm binding. IOHK `extvm` gRPC bridge hibernated (`vm.mode` defaults internal) | **besu (JVM)** + **revm (Rust)** — the two "reusable EVM library + per-fork spec selector" references |
| **Interpreter loop** | iterative `EVM.Run` loop, jump-table index per step; PC is `uint64` (deliberate YP deviation) | same geth iterative loop | iterative `runToHalt` — hand-tuned inlined `switch` for hot opcodes + registry fallback (intentionally un-OO) | iterative geth loop + `sync.Pool`-pooled call contexts, interned keys, generation-counter cache | **iterative frame loop** over managed `Stack<VmState>` (no CLR recursion) → 1024-depth safe; `unsafe`+`fixed`+`delegate*` dispatch | revm handler-based loop (external) | **`@tailrec exec` over an IMMUTABLE `ProgramState`** — `copy`-per-opcode, no mutable frame/pooling/`unsafe` (`VM.scala:228-256`, `ProgramState.scala:76-197`). Dispatch = `byteToOpCode.get(byte)` map probe. Stack/call depth 1024 enforced (`OpCode.scala:253-254`, `EvmConfig.scala:25`). The one pure-functional loop among the seven | **go-ethereum** / **besu** iterative loop is the mainstream shape; recursion is nobody's choice |
