...
/** Super-trait of every concern-specific proposal descriptor (EVM opcode/fee, reward policy, base-fee routing, …).
  ...
  *                  (Batch 5 framework §1.2 "Proposal dependencies"). Default empty. In Row 5.2 this is not yet
  *                  expanded by a `ForkSchedule` builder (that arrives in 5.3); it is declared so composition is
  *                  explicit and machine-checkable, and so ECIP-1121 can name the shared EIP impls it bundles.
