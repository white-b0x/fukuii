...
// Arithmetic over the BN128 pairing field tower, following
// https://eprint.iacr.org/2010/354.pdf — 'High-Speed Software Implementation of the Optimal Ate
// Pairing over Barreto–Naehrig Curves'. Ported for byte-behaviour from ethereumj / libff; the
// EIP-196/197 alt-bn128 precompiles are defined against exactly this tower.
...
object Fp:
  ...
  given fpField: FiniteField[Fp] with
    def zero: Fp = Fp(BigInt(0))
    def one: Fp = Fp(BigInt(1))
    def add(a: Fp, b: Fp): Fp = Fp((a.inner + b.inner).mod(P))
    def mul(a: Fp, b: Fp): Fp = Fp((a.inner * b.inner).mod(P))
    def sub(a: Fp, b: Fp): Fp = Fp((a.inner - b.inner).mod(P))
    def inv(a: Fp): Fp = Fp(a.inner.modInverse(P))
    def neg(a: Fp): Fp = Fp((P - a.inner).mod(P))
    def isValid(a: Fp): Boolean = a.inner >= 0 && a.inner < P
    def isZero(a: Fp): Boolean = a.inner == BigInt(0)

case class Fp2(a: Fp, b: Fp) extends FieldElement

object Fp2:

  // Twist non-residue for Fp2.
  ...
  given fp2Field: FiniteField[Fp2] with
    def one: Fp2 = Fp2(FiniteField[Fp].one, FiniteField[Fp].zero)
    def zero: Fp2 = Fp2(FiniteField[Fp].zero, FiniteField[Fp].zero)
    def add(a: Fp2, b: Fp2): Fp2 = Fp2(a.a + b.a, a.b + b.b)
    def sub(a: Fp2, b: Fp2): Fp2 = Fp2(a.a - b.a, a.b - b.b)
    def mul(a: Fp2, b: Fp2): Fp2 =
      ...
      Fp2(ra, rb)
    def neg(a: Fp2): Fp2 = Fp2(a.a.negated(), a.b.negated())
    def inv(a: Fp2): Fp2 =
      val t0 = a.a.squared()
      val t1 = a.b.squared()
      ...
      Fp2(ra, rb)
    def isValid(a: Fp2): Boolean = a.a.isValid() && a.b.isValid()
    def isZero(a: Fp2): Boolean = a == zero
...
object Fp6:
  ...
  // Alg.12 in the optimal ate-pairing paper.
  ...
  given fp6Field: FiniteField[Fp6] with
    def one: Fp6 = Fp6(FiniteField[Fp2].one, FiniteField[Fp2].zero, FiniteField[Fp2].zero)
    def zero: Fp6 = Fp6(FiniteField[Fp2].zero, FiniteField[Fp2].zero, FiniteField[Fp2].zero)
    def add(a: Fp6, b: Fp6): Fp6 = Fp6(a.a + b.a, a.b + b.b, a.c + b.c)
    def sub(a: Fp6, b: Fp6): Fp6 = Fp6(a.a - b.a, a.b - b.b, a.c - b.c)
    def mul(a: Fp6, b: Fp6): Fp6 =
      val t0 = a.a * b.a
      val t1 = a.b * b.b
      val t2 = a.c * b.c
      ...
      Fp6(ra, rb, rc)
    def neg(a: Fp6): Fp6 = Fp6(a.a.negated(), a.b.negated(), a.c.negated())
    def inv(a: Fp6): Fp6 =
      val t0 = a.a.squared()
      val t1 = a.b.squared()
      val t2 = a.c.squared()
      ...
      Fp6(c0 * t6, c1 * t6, c2 * t6)
    def isValid(a: Fp6): Boolean = a.a.isValid() && a.b.isValid() && a.c.isValid()
    def isZero(a: Fp6): Boolean = a == zero
...
object Fp12:
  ...
  private def cyclotomicExp(a: Fp12, exp: BigInt): Fp12 =
    var result = fp12Field.one
    var i = exp.bitLength - 1
    while i >= 0 do
      result = cyclotomicSquared(result)
      if exp.testBit(i) then result = result * a
      i = i - 1
    result
  ...
  given fp12Field: FiniteField[Fp12] with
    def one: Fp12 = Fp12(FiniteField[Fp6].one, FiniteField[Fp6].zero)
    def zero: Fp12 = Fp12(FiniteField[Fp6].zero, FiniteField[Fp6].zero)
    def add(a: Fp12, b: Fp12): Fp12 = Fp12(a.a + b.a, a.b + b.b)
    def sub(a: Fp12, b: Fp12): Fp12 = Fp12(a.a - b.a, a.b - b.b)
    def mul(a: Fp12, b: Fp12): Fp12 =
      val t0 = a.a * b.a
      val t1 = a.b * b.b
      val c0 = t0 + Fp6.mulByNonResidue(t1)
      val c1 = (a.a + a.b) * (b.a + b.b) - t0 - t1
      Fp12(c0, c1)
    def neg(a: Fp12): Fp12 = Fp12(a.a.negated(), a.b.negated())
    def inv(a: Fp12): Fp12 =
      val t0 = a.a.squared()
      val t1 = a.b.squared()
      ...
      val rb = (a.b * t3).negated()
      Fp12(ra, rb)
    def isValid(a: Fp12): Boolean = a.a.isValid() && a.b.isValid()
    def isZero(a: Fp12): Boolean = a == zero
