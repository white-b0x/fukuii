...
| `modules/storage` (L2) — **built** | [`modules/storage/AGENTS.md`](modules/storage/AGENTS.md) | [`03-L2-storage-trie.md`](docs/architecture/fukuii-rebuild/implementation-reports/03-L2-storage-trie.md) — byte-pure `DataSource`/RocksDB + `enum Namespace`, `StorageProfile`, `INodeStorage` scheme-indirection `(owner,path)`, `ChainWeight` + BUG-W7, composable pruning, `ColdStore`/era1 |
| `modules/trie` (L2) — **built** | [`modules/trie/AGENTS.md`](modules/trie/AGENTS.md) | [`03-L2-storage-trie.md`](docs/architecture/fukuii-rebuild/implementation-reports/03-L2-storage-trie.md) — MPT (`enum MptNode`, state-root core), `HexPrefix`, `MptStorage` seam, root-neutral path threading, `TrieLog` leaf-diff |
| `modules/evm` (L3) — **built** | [`modules/evm/AGENTS.md`](modules/evm/AGENTS.md) | [`04-L3-evm.md`](docs/architecture/fukuii-rebuild/implementation-reports/04-L3-evm.md) — sealed `OpCode` + per-fork `given GasCalculator`, `ProposalId` fold → single `forBlock(header, schedule)`, dense `IArray` + `validate`, `@tailrec` interpreter + call/create, precompiles→L0 crypto (ETC excludes KZG), branch-free `NoTracing` + role tracers, neutral EIP-keyed spine. **All byte-consensus — forge/beacon co-sign vs reference clients** |
| `modules/execution` (L4) — **built** | [`modules/execution/AGENTS.md`](modules/execution/AGENTS.md) | [`05-L4-execution.md`](docs/architecture/fukuii-rebuild/implementation-reports/05-L4-execution.md) — immutable `ProtocolSpec` bundle wrapping L3's single `forBlock`, family-agnostic block loop with the reward as the sole economics seam (`RewardScheme`, ECIP-1017 core-geth+besu-etc byte-exact era math, ECIP-1111 base-fee floor/treasury), `RequestType`→processor map (EIP-4895/6110/7002/7251), per-block `BlockExecutionOutcome`+state-diff, the `ledger↔consensus` DAG-cycle fix. **All byte-consensus — forge/beacon co-sign vs reference clients, banksy required consult on ECIP-1111** |
