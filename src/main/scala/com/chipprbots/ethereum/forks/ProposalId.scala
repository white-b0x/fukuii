...
/** L1 identity for a protocol proposal (Batch 5 framework §1.2).
  ...
  * **family-agnostic** — chain id keys the *config* layer (L2), never the implementation. `Eip(1559)` is one shared impl
  * referenced by every family that composes it, not a per-chain copy (§1.1 "share the implementation, never the
  * bundle").
  */
enum ProposalId:
